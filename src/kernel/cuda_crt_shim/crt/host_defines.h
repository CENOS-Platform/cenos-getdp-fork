...
//
// This file is NOT GetDP source: it's NVIDIA's own crt/host_defines.h from the CUDA
// Toolkit v13.2 installer (unmodified below this banner), vendored here
// because the pip-distributed "nvidia-cuda-runtime" wheel (version 13.1.80,
// as actually shipped alongside the RF app - see CMakeLists.txt,
// ENABLE_CUDSS) omits its own copy of include/crt/*.h entirely: its flat
// include/host_defines.h is only a deprecation-forwarding stub that itself does
// "#include \"crt/host_defines.h\"", expecting to find the real file here - which
// this wheel just doesn't ship.
//
// Host-only builds (anything not compiled by nvcc, i.e. this entire GetDP
// build - see LinAlg_CUDSS.cpp) only ever need this file and host_config.h
// out of crt/: cuda_runtime.h's other crt/*.h includes (device_functions.h,
// common_functions.h, the multi-hundred-KB device math-intrinsics headers)
// are all guarded by "#if defined(__CUDACC__)", which is only ever defined
// when nvcc itself is compiling - never true here, so they're not needed and
// were deliberately NOT vendored.
//
// Cross-version note: this is CUDA 13.2's copy of the file, used to stand in
// for the missing CUDA 13.1.80 one (the two versions were adjacent minor
// releases at the time this was written) - low risk since this file is
// pure host-compiler-version detection and macro definitions, not anything
// that varies with the actual CUDA runtime/driver version. If a future
// nvidia-cuda-runtime release ships its own crt/ directory, prefer deleting
// this shim over keeping it.
...
#ifndef __NV_TL_BUILTIN__
#define __NV_TL_BUILTIN__
#endif

#if defined(__aarch64__) && defined(__GNUC__) && (__GNUC__ >= 12 || (__GNUC__ == 11 && __GNUC_MINOR__ >= 4))
#define __CLZ_RETURN_TYPE unsigned int
#define __CLZ_PARAMETER_TYPE unsigned int 
#define __CLZLL_PARAMETER_TYPE unsigned long int 
#else
